...
<template>
  <div class="fixed inset-0 app-grid app-mobile-grid sm:app-desktop-grid p-4">
    <UiToolbar class="[grid-area:header]">
      <ToolbarButton @click="timesStore.copyTimesToClipboard">
        <svg
      ...
      <ToolbarButton @click="timesStore.importTimesFromClipboard">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
      ...
      <template v-if="timesStore.importData.length > 0">
        {{ timesStore.importData.length }} {{ timesStore.importData.length === 1 ? 'Eintrag' : 'Einträge' }} importieren?
        <ToolbarButton @click="timesStore.applyImport()">
          Zusammenführen
        </ToolbarButton>
        <ToolbarButton @click="timesStore.applyImport(true)">
          Ersetzen
        </ToolbarButton>
        <ToolbarButton @click="timesStore.abortImport()">
          Abbrechen
        </ToolbarButton>
      </template>
    </UiToolbar>
